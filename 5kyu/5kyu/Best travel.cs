...
//The function chooseBestSum (or choose_best_sum or ... depending on the language) will take as parameters t (maximum sum of distances, integer >= 0), k(number of towns to visit, k >= 1) and ls(list of distances, all distances are positive or zero integers and this list has at least one element).
...
public static class SumOfK
{
    ...
    private static int Combinations(int t, int k, List<int> ls, int i)
    {
        if (k == 0 && t >= 0)
        {
            return 0;
        }
        else if (k < 0 || i >= ls.Count)
        {
            return Int32.MinValue;
        }
        else
        {
            return Math.Max(Combinations(t, k, ls, i + 1), ls[i] + Combinations(t - ls[i], k - 1, ls, i + 1));
        }
    }
}